...
static void i2s_rx_block_complete(uint32_t *rx_buf_released)
{
    ...
    err = audio_i2s_set_next_rx_buf(rx_buf_released);
    if (err) {
        if ((i2s_requeue_err_cnt++ % 50U) == 0U) {
            printk("audio_i2s_set_next_rx_buf failed: %d (cnt=%u)\n", err,
                   i2s_requeue_err_cnt);
        }
    }
}
...
static int i2s_capture_start(void)
{
    ...
    audio_i2s_rx_cb_register(i2s_rx_block_complete);
    ...
    err = audio_i2s_start(i2s_rx_buf_a);
    if (err) {
        printk("audio_i2s_start failed: %d\n", err);
        return err;
    }

    err = audio_i2s_set_next_rx_buf(i2s_rx_buf_b);
    if (err) {
        printk("audio_i2s_set_next_rx_buf failed: %d\n", err);
        return err;
    }

    printk("I2S RX started via NRFX API (ACLK + DTS pinctrl)\n");
    return 0;
}
