...
static audio_i2s_rx_cb_t rx_cb;

static void i2s_comp_handler(nrfx_i2s_buffers_t const *released_bufs, uint32_t status)
{
	if ((status == NRFX_I2S_STATUS_NEXT_BUFFERS_NEEDED) && released_bufs && rx_cb &&
	    released_bufs->p_rx_buffer) {
		rx_cb(released_bufs->p_rx_buffer);
	}
}

void audio_i2s_rx_cb_register(audio_i2s_rx_cb_t cb)
{
	rx_cb = cb;
}
...
int audio_i2s_start(uint32_t *rx_buf_initial)
{
	nrfx_i2s_buffers_t i2s_buf;
	nrfx_err_t ret;

	if (state != AUDIO_I2S_STATE_IDLE || rx_buf_initial == NULL) {
		return -EINVAL;
	}

	i2s_buf.p_rx_buffer = rx_buf_initial;
	i2s_buf.p_tx_buffer = NULL;
...
int audio_i2s_set_next_rx_buf(uint32_t *rx_buf)
{
	nrfx_i2s_buffers_t i2s_buf;
	nrfx_err_t ret;

	if (state != AUDIO_I2S_STATE_STARTED || rx_buf == NULL) {
		return -EINVAL;
	}
